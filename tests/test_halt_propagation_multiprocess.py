...
class TestHaltPropagation:
    ...
    def test_halt_reaches_all_workers(self, dispatcher, worker_config):
        ...
        assert (
            halt_result.acks_received == NUM_WORKERS
    ...
    def test_halt_cascade_within_slo(self, dispatcher, worker_config):
        ...
        assert (
            elapsed_ms < HALT_CASCADE_SLO_MS
        ...
        print("  STATUS: PASS (INV-HALT-2 satisfied)")
    ...
    def test_no_orphan_workers_after_halt(self, dispatcher, worker_config):
        ...
        assert len(sessions) == 0, f"Orphan sessions found: {[s.session_id for s in sessions]}"
    ...
    def test_halt_per_worker_latency(self, dispatcher, worker_config):
        ...
        for ack in halt_result.worker_acks:
            assert ack.latency_ms < 100, f"Worker {ack.worker_id} ack took {ack.latency_ms:.2f}ms"
...
class TestHaltStress:
    ...
    def test_halt_propagation_many_workers(self, dispatcher):
        ...
        config = WorkerConfig(worker_type=WorkerType.GENERIC, command="sleep 300")
        ...
        assert (
            halt_result.total_latency_ms < HALT_CASCADE_SLO_MS
...
class TestInvariantProof:
    ...
    def test_inv_halt_2_proof(self, dispatcher, worker_config):
        ...
        assert max_latency < HALT_CASCADE_SLO_MS, f"INV-HALT-2 violated: max={max_latency}ms"
