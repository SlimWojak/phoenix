...
try:
    from tests.chaos.chaos_suite_v2 import CHAOS_VECTORS as V2_VECTORS
    ...
    from chaos_suite_v2 import CHAOS_VECTORS as V2_VECTORS
...
class ChaosSuiteV3(ChaosSuiteV2):
    ...
    def _validate_price_data(self, df: "pd.DataFrame", symbol: str = "EURUSD") -> dict:
        ...
        if nan_count > 0:
            validation["violations"].append(
                {"type": "NAN_IN_PRICE", "count": int(nan_count), "severity": "HIGH"}
            )
        ...
        if inf_count > 0:
            validation["valid"] = False
            validation["violations"].append(
                {"type": "INF_IN_PRICE", "count": int(inf_count), "severity": "CRITICAL"}
            )
    ...
    def _test_V3_RIVER_003(self, vector: ChaosVector) -> VectorTestResult:
        ...
        stress_data = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    "2025-01-15", periods=stress_volume, freq="600us", tz="UTC"
                ),
        ...
        if halt_latencies:
            ...
            p99_latency = (
                sorted(halt_latencies)[int(len(halt_latencies) * 0.99)]
                if len(halt_latencies) > 10
                else max_latency
            )
        else:
...
def generate_report(suite: ChaosSuiteV3, results: dict) -> str:
    ...
    report.append(f"**Date:** {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')} UTC")
    report.append("**Sprint:** S28.A")
    report.append("")
    ...
    gates = results["gates"]
    report.append(
        f"| GATE_1_SURVIVAL | >90% vectors pass | {'✓' if gates['GATE_1_SURVIVAL'] else '✗'} |"
    )
    report.append(
        f"| GATE_2_NO_CRITICAL | Zero CRITICAL failures | {'✓' if gates['GATE_2_NO_CRITICAL'] else '✗'} |"
    )
    report.append(
        f"| GATE_3_NO_REGRESSION | V2 still 100% | {'✓' if gates['GATE_3_NO_REGRESSION'] else '✗'} |"
    )
    report.append("")
    ...
    for result in suite.v3_results:
        ...
        if vector:
            ...
            report.append(
                f"| {vector.id} | {vector.name} | {vector.injection[:30]}... | {result.result.value} | {notes} |"
            )
    ...
    for result in suite.v3_results:
        if result.result == VectorResult.FAIL and "gap" in result.evidence:
            gaps.append({"vector": result.vector_id, "gap": result.evidence["gap"]})
