...
def dispatcher():
    ...
    config = DispatcherConfig(
        dispatcher_id="test-dispatcher", tmux_session_prefix="phoenix-test", max_workers=10
    )
    d = Dispatcher(config)
    d.start()
    yield d
...
class TestDispatcherInit:
    ...
    def test_dispatcher_starts(self):
        ...
        config = DispatcherConfig(
            dispatcher_id="init-test", tmux_session_prefix="phoenix-init-test"
        )
        d = Dispatcher(config)
...
class TestCleanup:
    ...
    def test_stop_kills_all_workers(self, worker_config):
        ...
        config = DispatcherConfig(
            dispatcher_id="cleanup-test", tmux_session_prefix="phoenix-cleanup-test"
        )
        d = Dispatcher(config)
        d.start()
