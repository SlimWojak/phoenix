...
def create_test_topology():
    ...
    # Wire dependencies
    river.register_dependent("enrichment", lambda halt_id: enrichment.acknowledge_halt(halt_id))
    enrichment.register_dependent("cso", lambda halt_id: cso.acknowledge_halt(halt_id))
    cso.register_dependent("execution", lambda halt_id: execution.acknowledge_halt(halt_id))

    return {"river": river, "enrichment": enrichment, "cso": cso, "execution": execution}
