...
class TestBeadEmission:
    ...
    def test_critical_emits_bead(self):
        ...
        manager = AlertManager(bead_callback=lambda b: beads_received.append(b))

        # Emit CRITICAL alert
        alert = manager.emit(
            AlertClass.HALT_LATENCY, AlertLevel.CRITICAL, "Halt latency > 50ms", source_id="test"
        )
    ...
    def test_warn_no_bead(self):
        ...
        # Emit WARN alert
        alert = manager.emit(
            AlertClass.HALT_LATENCY, AlertLevel.WARN, "Halt latency > 10ms", source_id="test"
        )
...
class TestAutoHaltEscalation:
    ...
    def test_no_auto_halt_under_threshold(self):
        ...
        manager = AlertManager(
            halt_callback=lambda r: halt_called.__setitem__(0, True), debounce_seconds=0
        )
    ...
    def test_auto_halt_reset(self):
        ...
        manager = AlertManager(halt_callback=lambda r: None, debounce_seconds=0)
...
def run_all_tests():
    ...
    print("GATE_D2_BEAD_EMISSION: 3/3 (PASS)")
    print("GATE_D3_AUTO_HALT: 4/4 (PASS)")
    print("-" * 60)
