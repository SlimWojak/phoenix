...
class TestGateC1PaperTrades:
    ...
    def test_position_to_pnl(self):
        ...
        exit_result = broker.exit_position(
            position_id=result.position_id, exit_price=1.1010, reason="take_profit"
        )
    ...
    def test_full_cycle(self):
        ...
        for i, trade in enumerate(trades):
            ...
            exit_result = broker.exit_position(
                position_id=order_result.position_id, exit_price=trade["exit"]
            )
            assert exit_result.success
...
class TestGateC2Deterministic:
    ...
    def test_deterministic_replay(self):
        ...
        signals = [
            MockSignal("S1", datetime.now(UTC), "ENTRY_LONG", "TEST/USD", 1.0, 1.0),
            MockSignal("S2", datetime.now(UTC), "EXIT", "TEST/USD", 1.01),
            MockSignal("S3", datetime.now(UTC), "ENTRY_SHORT", "TEST/USD", 1.01, 1.0),
            MockSignal("S4", datetime.now(UTC), "EXIT", "TEST/USD", 0.99),
        ]
    ...
    def test_determinism_verifier(self):
        ...
        verification = DeterminismVerifier.verify(
            harness_factory=harness_factory, signals=signals, runs=3
        )

        assert verification["deterministic"], f"Non-deterministic: {verification['unique_hashes']}"
...
class TestPnLCalculator:
    ...
    def test_long_profit(self):
        ...
        pnl = PnLCalculator.calculate_pnl(
            entry_price=1.1000, exit_price=1.1020, size=1.0, direction="LONG"
        )
    ...
    def test_long_loss(self):
        ...
        pnl = PnLCalculator.calculate_pnl(
            entry_price=1.1000, exit_price=1.0980, size=1.0, direction="LONG"
        )
        assert abs(pnl - (-0.002)) < 0.0001
    ...
    def test_short_profit(self):
        ...
        pnl = PnLCalculator.calculate_pnl(
            entry_price=1.1000, exit_price=1.0980, size=1.0, direction="SHORT"
        )
        assert abs(pnl - 0.002) < 0.0001
    ...
    def test_short_loss(self):
        ...
        pnl = PnLCalculator.calculate_pnl(
            entry_price=1.1000, exit_price=1.1020, size=1.0, direction="SHORT"
        )
        assert abs(pnl - (-0.002)) < 0.0001
