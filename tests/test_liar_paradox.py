...
def run_liar_paradox_test() -> LiarParadoxResult:
    ...
    df_corrupted, injection_meta = teller.inject_corruption(
        df_clean, bar_index=injection_bar, field=INJECTION_FIELD, delta_pips=INJECTION_DELTA_PIPS
    )
    ...
    hash_mismatch_count = sum(1 for a in integrity_result.anomalies if a["type"] == "HASH_MISMATCH")
