...
from .types import CommandResult, SessionId, SessionInfo
...
class TmuxController:
    ...
    def create_session(
        self, name: str, command: str | None = None, working_dir: str | None = None
    ) -> SessionId:
        """
    ...
    def send_command(
        self, session_id: SessionId, command: str, enter: bool = True
    ) -> CommandResult:
        """
    ...
    def send_signal(self, session_id: SessionId, signal: str = "SIGTERM") -> bool:
        """
        ...
        cmd_result = self.send_command(session_id, f"kill -{signal} $$", enter=True)
    ...
    def get_session_output(self, session_id: SessionId, lines: int = 100) -> str | None:
        """
                    ...
                    if name.startswith(self.session_prefix):
                        sessions.append(
                            SessionInfo(
                                ...
                                created_at=datetime.fromtimestamp(int(parts[2]), tz=UTC)
                                if parts[2].isdigit()
                                else None,
                                attached=parts[3] == "1",
                            )
