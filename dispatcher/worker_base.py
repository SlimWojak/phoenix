...
class WorkerBase(GovernanceInterface):
    ...
    def __init__(
        ...
        parent_dispatcher: str | None = None,
    ):
        super().__init__()
        ...
        self._heartbeat_emitter = HeartbeatEmitter(
            worker_id=worker_id, interval_ms=heartbeat_interval_ms, send_fn=self._send_heartbeat
        )
    ...
    @property
    def parent_dispatcher(self) -> str | None:
    ...
    def get_degradation_paths(self) -> dict[str, DegradationAction]:
        return {
            "WORKER-FAIL-3": DegradationAction(
                action_type="retry_with_backoff", params={"max_retries": 3, "backoff_ms": 1000}
            ),
        }
...
class MockWorker(WorkerBase):
    ...
    def __init__(self, worker_id: WorkerId, loop_delay_ms: int = 100):
        super().__init__(worker_id=worker_id, worker_type=WorkerType.GENERIC, tier=ModuleTier.T0)
