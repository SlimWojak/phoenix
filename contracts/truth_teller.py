...
class TruthTeller:
    ...
    def verify_integrity(
        self, df: pd.DataFrame, reference_hashes: dict[str, str] | None = None
    ) -> IntegrityResult:
        """
    ...
    def inject_corruption(
        self, df: pd.DataFrame, bar_index: int, field: str = "close", delta_pips: float = 1.0
        ...
        timestamp = (
            df.iloc[bar_index]["timestamp"] if "timestamp" in df.columns else df.index[bar_index]
        )
        ...
        injection_meta = {
            ...
            "original_hash": self.compute_bar_hash(
                pd.Series(
                    {
                        "timestamp": timestamp,
...
class ChaosBunny:
    ...
    def detect_chaos(self, df: pd.DataFrame, current_time: datetime | None = None) -> ChaosResult:
        """
        ...
        chaos_vectors_active = sum(
            [gap_count > 0, latency_detected, spike_count > 0, sequence_errors > 0]
        )
    ...
    def _detect_latency(self, df: pd.DataFrame, current_time: datetime) -> tuple[list[dict], bool]:
        """
    ...
    def inject_gaps(
        self, df: pd.DataFrame, gap_positions: list[int], gap_duration_minutes: int = 15
    ) -> tuple[pd.DataFrame, dict]:
        """
    ...
    def inject_latency(
        self, df: pd.DataFrame, shift_seconds: int = 120
    ) -> tuple[pd.DataFrame, dict]:
        """
        ...
        return df, {"type": "LATENCY_INJECTION", "shift_seconds": shift_seconds}

    def inject_spike(
        self, df: pd.DataFrame, bar_index: int, spike_pips: float = 50.0
    ) -> tuple[pd.DataFrame, dict]:
        """
    ...
    def inject_sequence_error(
        self, df: pd.DataFrame, swap_indices: tuple[int, int]
    ) -> tuple[pd.DataFrame, dict]:
        """
        ...
        return df, {"type": "SEQUENCE_INJECTION", "swapped_indices": swap_indices}
    ...
    def inject_correlated_chaos(
        ...
        df_chaos, gap_meta = self.inject_gaps(
            df, gap_positions=[len(df) // 3], gap_duration_minutes=10
        )

        # Add spike
        df_chaos, spike_meta = self.inject_spike(
            df_chaos, bar_index=len(df_chaos) // 2, spike_pips=60.0
        )

        # Add latency
        df_chaos, latency_meta = self.inject_latency(df_chaos, shift_seconds=120)
