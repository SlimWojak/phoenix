# 🔥 THE PHOENIX MANIFESTO — A Sovereign Refinery Tale
...
**Evolution:** Gas Town was the forge that built tools. Phoenix is what the tools protect — a trading intelligence that learns, remembers, and governs itself.
...
- **Must Never:** Be ignored. Be "improved upon" by machines. Have her methodology diluted by optimization. Be asked to repeat herself — the CSO should remember.
...
### 🎭 THE CSO (Chief Strategy Officer) — **(PHOENIX-RUNTIME)**
...
### 📜 THE CHRONICLER (INV-CONTRACT-1) — **(PHOENIX-RUNTIME)**
...
### ⚡ THE KILL SWITCH (INV-CONTRACT-2) — **(PHOENIX-RUNTIME)**
...
### 🔍 THE TRUTH TELLER (INV-CONTRACT-3) — **(PHOENIX-RUNTIME)**
...
### 📋 THE NOTARY (INV-CONTRACT-4) — **(PHOENIX-RUNTIME)**
...
### 📢 THE HERALD (INV-CONTRACT-5) — **(PHOENIX-RUNTIME)**
...
### 🌊 THE RIVER (Data Bus) — **(INFRASTRUCTURE)**
...
- **Grudge:** NEX had two rivers — one for backtests, one for live. They diverged. Trust died.
- **The Doctrine:** "If the blood is poisoned, the brain's decisions are irrelevant."

### 📿 THE BEADS (History/Memory) — **(INFRASTRUCTURE)**
**Narrative Identity:** The Dynasty's Memory.

- **The Vibe:** Every decision, every insight, every certified observation — strung on a chain that stretches back to the beginning and forward to eternity. Cannot be erased. Cannot be altered. Can only be added to.
...
### 🏦 THE VAULT (Protected Parameters) — **(INFRASTRUCTURE)**
...
### 🗼 THE WATCHTOWER (TMUX) — **(INFRASTRUCTURE)**
...
### 🕯️ THE INQUISITOR (Grand Zealot) — **(FORGE)**
...
### ⚔️ THE GATE GUARD (SignatureGate) — **(FORGE)**
...
- **The Rule:** Convergence ≠ Correctness. The code must match the locked signature exactly.

### 🏟️ THE ARENA (Scrutiny Gauntlet) — **(FORGE)**
...
### 🦉 THE WISE OWL (Gemini) — **(HIVE-WORKFLOW)**
...
### 🐗 THE CHAOS BOAR (Grok) — **(HIVE-WORKFLOW)**
...
### ☕ THE NIGHT FOREMAN (Orchestrator) — **(HIVE-WORKFLOW)**
...
### 🗺️ THE CARTOGRAPHER (SpecArchitect)
...
- **Responsibility:** Vision → Specs. DAG definition. Make the implicit explicit.
- **Must Never:** Let ambiguity pass. Ship specs without invariants.

### 🔨 THE BLACKSMITH (Materializer)
...
- **Responsibility:** Intent → Artifacts. 1.0 Fidelity.
...
### 🤡 THE BUNNY (Adversarial Fuzzer) — **(FORGE)**
...
- **S24 Mandate:** Chaos suite must include market fault injection — not just code fuzzing. If the system hasn't been nuked by fake regime shifts, it hasn't been tested.

### 💀 THE REAPER (BeadReaper)
...
|-----------|------------------------|
| `river_health_degraded` | The River is polluted — Truth Teller screaming |
| `halt_signal_received` | Kill Switch pulled — everything stops in <50ms |
...
| `doctrine_conflict` | Olya contradicted locked law — ceremony required |
