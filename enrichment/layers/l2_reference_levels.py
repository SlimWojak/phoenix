...
PIP_MULTIPLIERS = {
    ...
    "AUDJPY": 100, "CADJPY": 100, "CHFJPY": 100, "NZDJPY": 100,
}
...
def enrich(df: pd.DataFrame, symbol: str = None) -> pd.DataFrame:
    ...
    pip_mult = PIP_MULTIPLIERS.get(symbol.upper(), DEFAULT_PIP_MULTIPLIER) if symbol else DEFAULT_PIP_MULTIPLIER
...
def _calculate_asia_range(df: pd.DataFrame, pip_mult: int) -> pd.DataFrame:
    ...
    asia_stats = df[asia_mask].groupby('trading_day').agg({
        'high': 'max',
        'low': 'min'
...
def _calculate_pdh_pdl(df: pd.DataFrame, pip_mult: int) -> pd.DataFrame:
    ...
    """
    # Daily stats
    daily_stats = df.groupby('trading_day').agg({
        'high': 'max',
        'low': 'min'
...
def _calculate_weekly_levels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Weekly stats
    weekly_stats = df.groupby('_week_id').agg({
        'high': 'max',
        'low': 'min'
...
def _calculate_session_levels(
    ...
    # Groupby
    session_stats = df[session_mask].groupby('trading_day').agg({
        'high': 'max',
        'low': 'min'
    })
...
LAYER_2_COLUMNS = [
    # Asia (9)
    'asia_high', 'asia_low', 'asia_range', 'asia_range_pips', 'asia_range_ce',
    'above_asia_range', 'below_asia_range', 'inside_asia_range', 'price_vs_asia_ce',
    # PDH/PDL (9)
    'pdh', 'pdl', 'pd_range', 'pd_range_pips', 'pd_ce',
    'above_pdh', 'below_pdl', 'between_pd_levels', 'price_vs_pd_ce',
    # Weekly (11)
    'pwh', 'pwl', 'pw_range', 'pw_ce',
    'above_pwh', 'below_pwl', 'between_pw_levels', 'price_vs_pw_ce',
    # London (7)
    'lon_session_high', 'lon_session_low', 'lon_session_range', 'lon_session_range_pips',
    'above_london_high', 'below_london_low', 'inside_london_range',
    # NY (7)
    'ny__session_high', 'ny__session_low', 'ny__session_range', 'ny__session_range_pips',
    'above_ny_high', 'below_ny_low', 'inside_ny_range',
    # Stubbed (28)
    'pd_fvg_bull_high', 'pd_fvg_bull_low', 'pd_fvg_bull_ce', 'in_pd_fvg_bull',
    'pd_fvg_bear_high', 'pd_fvg_bear_low', 'pd_fvg_bear_ce', 'in_pd_fvg_bear',
    'pd_fvg_count_bull', 'pd_fvg_count_bear',
    'pd_ob_bull_high', 'pd_ob_bull_low', 'in_pd_ob_bull',
    'pd_ob_bear_high', 'pd_ob_bear_low', 'in_pd_ob_bear',
    'ifvg_bull_high', 'ifvg_bull_low', 'in_ifvg_bull',
    'ifvg_bear_high', 'ifvg_bear_low', 'in_ifvg_bear',
    'bpr_high', 'bpr_low', 'bpr_mid', 'in_bpr', 'bpr_type', 'bpr_strength',
]
