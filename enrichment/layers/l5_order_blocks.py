...
def _detect_order_blocks(df: pd.DataFrame, lookback: int = 10) -> dict[str, list[dict]]:
    """
    Detect Order Blocks.
    ...
    if "is_displacement" in df.columns:
        ...
        disp_up = (
            df["displacement_up"].values
            if "displacement_up" in df.columns
            else (closes > opens) & is_disp
        )
        disp_down = (
            df["displacement_down"].values
            if "displacement_down" in df.columns
            else (closes < opens) & is_disp
        )
    else:
    ...
    for i in range(n):
        ...
        if i in bull_ob_map:
            ...
            active_bull_obs.append(
                {"high": ob["high"], "low": ob["low"], "ce": ob["ce"], "start_idx": i, "touches": 0}
            )
