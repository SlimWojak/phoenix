...
from .position import Position, PositionState, PositionRegistry
from .intent import ExecutionIntent, IntentType, IntentStatus, Direction
...
class PaperBrokerStub:
    ...
    def __init__(
        ...
        current_price_fn: Optional[Callable[[str], float]] = None
    ):
        """
        ...
        # Halt state
        self._halted = False
        self._halt_id: Optional[str] = None
    ...
    def _check_halt(self) -> None:
        ...
        if self._halt_check_fn():
            if not self._halted:
                self._halted = True
                self._halt_id = f"HALT-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
