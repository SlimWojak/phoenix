...
from .types import ErrorClassification, ErrorCategory, ErrorAction
...
class TierViolationError(GovernanceError):
    ...
    def __init__(self, module_tier: str, attempted_action: str, forbidden: str):
        ...
        super().__init__(
            f"Tier {module_tier} cannot {attempted_action}: {forbidden} is forbidden"
        )
...
class TokenStateMismatchError(ApprovalTokenError):
    ...
    def __init__(self, token_hash: str, current_hash: str):
        ...
        super().__init__(
            f"Token state_hash {token_hash} != current {current_hash}"
        )
...
class HaltBlocksActionError(ApprovalTokenError):
    ...
    def __init__(self, action: str, halt_id: str):
        ...
        self.halt_id = halt_id
        super().__init__(
            f"T2 action '{action}' blocked: halt_signal TRUE (halt_id={halt_id})"
        )


class InvariantViolationError(GovernanceError):
    """Raised when an invariant is violated."""
    def __init__(self, invariant_id: str, evidence: Optional[Dict] = None):
...
def classify_error(
    error: Exception,
    custom_rules: Optional[Dict[type, ErrorClassification]] = None
) -> ErrorClassification:
    """
