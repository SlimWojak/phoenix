...
class GovernanceInterface(ABC):
    ...
    def __init__(self):
        ...
        self._halt_manager: Optional[HaltManager] = None
    ...
    def report_violation(
        ...
        ticket = ViolationTicket(
            ticket_id=hashlib.sha256(
                f"{self.module_id}|{invariant_id}|{now}".encode()
            ).hexdigest()[:12],
    ...
    def validate_t2_action(
        ...
        validator = TokenValidator(
            halt_signal=self._halt_manager.signal,
            state_hash_fn=self.compute_state_hash
        )
