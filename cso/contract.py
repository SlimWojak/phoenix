...
class FourQGateResult:
    ...
    def all_pass(self) -> bool:
        return all(
            [self.q1_htf_order_flow, self.q2_dealing_range, self.q3_pda_destination, self.q4_timing]
        )
    ...
    def get_degradation_paths(self) -> dict[str, DegradationAction]:
        return {
            "CSO-FAIL-1": DegradationAction(
                action_type="emit_neutral", params={"reason": "warmup_incomplete"}
            ),
            "CSO-FAIL-2": DegradationAction(
                action_type="reduce_confidence", params={"multiplier": 0.5}
            ),
        }
